...
@section('content')
    ...
    <form method="POST" action="{{ route('venda.update', $venda->id) }}">
        ...
        <div class="mb-3 mt-3">
            ...
            <select class="form-select" id="brinquedo" name="brinquedo" onchange="updateValor()">
                @foreach ($brinquedos as $brinquedo)
                    <option {{ $venda->brinquedo_id == $brinquedo->id ? 'selected' : '' }}
                        valor_brinquedo="{{ $brinquedo->valor_ingresso }}" value="{{ $brinquedo->id }}">
                        {{ $brinquedo->nome }}
                    </option>
                @endforeach
        ...
        <div class="mb-3 mt-3">
            ...
            <select class="form-select" id="pagamento" name="pagamento">
                @foreach ($formasPagamento as $formaPagamento)
                    <option {{ $venda->forma_pagamento == $formaPagamento ? 'selected' : '' }}>{{ $formaPagamento }}
                    </option>
                @endforeach
        ...
        <div class="mb-3 mt-3">
            ...
            <input value="{{ $venda->quantidade_ingressos }}" onchange="updateValor()" type="number" class="form-control"
                id="quantidade" name="quantidade" step="1" value="1">
        </div>

        <div class="mb-3 mt-3">
            <label for="cpf">CPF:</label>
            <input value="{{ $venda->cpf }}" type="text" class="form-control" id="cpf"
                placeholder="Digite o cpf do cliente" name="cpf">
        </div>
